			...
			<main class="dc_container">
				<section class="left_content"> 
					...
					<%
						...
						User detailLoginUserInfo = (User)session.getAttribute("LOGIN_USER_INFO");
									...
									<div class="col-3 border border-2 mt-5 mb-4 bg-secondary bg-opacity-10">
										<div>
											...
											<%
												...
												if (detailLoginUserInfo != null) {
													if (detailLoginUserInfo.getNo() != board.getWriter().getNo()) {
														BoardLiker boardLiker = boardDao.getBoardLiker(no, detailLoginUserInfo.getNo());
														if (boardLiker == null) {
															canLike = true;
														}
													}
					...
					<div class="row">
						...
						<!-- div 기준으로 양옆으로 배치 -->
							<div class="col d-flex justify-content-between">
								...
								<%
									if(detailLoginUserInfo != null && detailLoginUserInfo.getNo() == board.getWriter().getNo()){
								%>
								<div>
									<a href="updateform.jsp?no=<%=board.getNo() %>&pageNo=<%=pageNo %>" class="btn btn-secondary">수정</a>
									<a href="delete.jsp?no=<%=board.getNo() %>&pageNo=<%=pageNo %>" class="btn btn-secondary">삭제</a>
								</div>
								<%
									}
								%>
				...
				%>
											...
											<!-- 답글달기 아이콘 -->
											<%
												if(detailLoginUserInfo!=null && comments.getOrder() == 0) {
											%>
												<i class="fa  fa-commenting-o" aria-hidden="true" onclick="toggleform('form-<%=comments.getNo() %>')"></i>
											<% } 
												// 댓글 삭제 아이콘							
												if(detailLoginUserInfo != null && detailLoginUserInfo.getNo() == comments.getWriter().getNo()) {
				...
				%>							
								...
								%>
										<div class="row my-3" id="form-<%=comments.getNo() %>" style="display: none;">
											<div class="col">
												<!-- comment_no, animal_board_no, comment_writer_no, comment_content, comment_order, comment_group -->
